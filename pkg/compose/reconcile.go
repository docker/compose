...
func Reconcile(project *types.Project, observed *ObservedState, opts ReconcileOptions) (*ReconciliationPlan, error) {
	...
	// in observed.Networks/Volumes. We scan all containers to find the actual
	// network IDs and volume names used at runtime.
...
func findContainersOnNetwork(observed *ObservedState, _ string, networkID string) []container.Summary {
...
func buildDependencyEdges(project *types.Project, plan *ReconciliationPlan) {
	// Index the "service ready" operation for each service name.
	...
	for id, op := range plan.Operations {
		switch op.Type {
		case OpStartContainer:
			// Start-after-rename is the final step of a recreate chain
		...
		case OpCreateContainer:
			// Fresh create (not part of a recreate chain — Existing is nil)
	...
	for _, op := range plan.Operations {
		// Add service/network/volume dependency edges for container ops
	...
	// Build reverse map (Dependents) for ALL operations with DependsOn,
	// including stop→remove chains from scale-down and orphan removal.
	for _, op := range plan.Operations {
