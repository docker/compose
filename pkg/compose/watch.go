...
func NewWatcher(project *types.Project, options api.UpOptions, w WatchFunc) *Watcher {
...
func (s *composeService) watch(ctx context.Context, project *types.Project, options api.WatchOptions) (func() error, error) { //nolint: gocyclo
	...
	if len(paths) == 0 {
		return nil, fmt.Errorf("none of the selected services is configured for watch, consider setting an 'develop' section")
	}
