...
func emitErrorEvent(node *PlanNode, events api.EventProcessor, err error) {
	...
	var id string
	switch {
	case op.Type == OpWaitCondition:
		// the user-facing identity, matching the Waiting/Skipped events the
		// wait itself emits — never the internal "wait:<dep>:<condition>" ID
		id = "Service " + op.Name
