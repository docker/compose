...
func (s *composeService) InspectState(ctx context.Context, project *types.Project) (*ObservedState, error) {
	...
	// Partition containers by service, excluding one-off containers
	// (e.g. from `compose run`) which should not affect scale/recreate decisions.
	containersByService := map[string]Containers{}
	for _, c := range allContainers.filter(isNotOneOff) {
		service := c.Labels[api.ServiceLabel]
		containersByService[service] = append(containersByService[service], c)
	}

	// Identify orphan containers (include one-offs so they get cleaned up)
