...
func checkClassicBuilderSupported(service types.ServiceConfig) error {
	if len(service.Build.Platforms) > 1 {
		return errors.New("the classic builder doesn't support multi-arch build; building with BuildKit requires the buildx Docker CLI plugin, and DOCKER_BUILDKIT must not be set to 0")
	}
	if service.Build.Privileged {
		return errors.New("the classic builder doesn't support privileged mode; building with BuildKit requires the buildx Docker CLI plugin, and DOCKER_BUILDKIT must not be set to 0")
	}
	if len(service.Build.AdditionalContexts) > 0 {
		return errors.New("the classic builder doesn't support additional contexts; building with BuildKit requires the buildx Docker CLI plugin, and DOCKER_BUILDKIT must not be set to 0")
	}
	if len(service.Build.SSH) > 0 {
		return errors.New("the classic builder doesn't support SSH keys; building with BuildKit requires the buildx Docker CLI plugin, and DOCKER_BUILDKIT must not be set to 0")
	}
	if len(service.Build.Secrets) > 0 {
		return errors.New("the classic builder doesn't support secrets; building with BuildKit requires the buildx Docker CLI plugin, and DOCKER_BUILDKIT must not be set to 0")
	}
	return nil
}
...
func prepareClassicBuildContext(specifiedContext, dockerfileName string, progBuff io.Writer) (*classicBuildContext, error) {
	...
	case build.ContextTypeStdin:
		return nil, errors.New("building from STDIN is not supported")
