...
func generateCommand(p *ProjectOptions, backend api.Service) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: Adapt(func(ctx context.Context, args []string) error {
			return runReverse(ctx, backend, opts, args)
		}),
	}
...
func runReverse(ctx context.Context, backend api.Service, opts reverseOptions, containers []string) error {
	_, _ = fmt.Fprintln(os.Stderr, "reverse command is EXPERIMENTAL")
	if len(containers) == 0 {
		return fmt.Errorf("at least one service must be specified")
	}
	project, err := backend.Generate(ctx, api.ReverseOptions{
